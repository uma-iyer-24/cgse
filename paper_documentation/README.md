...
| [`CGSE-implementation-details.md`](CGSE-implementation-details.md) | **Deep technical walkthrough** of the SEArch + CGSE-on-SEArch system (Tier 2 / Tier 3). Goes inside each module: channel-attention KD internals, stage-output node correspondence, modification-value scorer, candidate enumeration with safety filters, edge-splitting operators with function-preserving init, the unified outer loop, the per-candidate critic, the student probe (`act_var_ratio` via power iteration, `grad_l2`, `weight_delta`), REINFORCE with EMA baseline, optimizer-state preservation across mutations, YAML-key → code-path mapping, CSV/JSONL schemas, smoke-test expectations, performance characteristics, gotchas, and extension points (reversible operators, new probe features, transformer critic). |
